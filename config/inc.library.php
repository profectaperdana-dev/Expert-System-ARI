...
function R1($a, $b){
	$calcSql = "SELECT FORMAT(($a + 17 * $b)/18, 8)";
	$calcQry = mysql_query($calcSql) or die ("Error calcKali ($a * $b) :".mysql_error());
	$calcHsl = mysql_fetch_array($calcQry);
	$hasil = $calcHsl[0];
	return $hasil;
}
...
function calcKali($a, $b) {
	$calcSql = "SELECT FORMAT($a * $b, 8)";
	$calcQry = mysql_query($calcSql) or die ("Error calcKali ($a * $b) :".mysql_error());
	$calcHsl = mysql_fetch_array($calcQry);
	$hasil = $calcHsl[0];
	return $hasil;
...
function calcTambah($a, $b) {
	$calcSql = "SELECT FORMAT($a + $b, 8)";
	$calcQry = mysql_query($calcSql) or die ("Error calcTambah :".mysql_error());
	$calcHsl = mysql_fetch_array($calcQry);
	$hasil = $calcHsl[0];
	return $hasil;
}
function calcBagi($a, $b) {
	$calcSql = "SELECT FORMAT($a / $b, 8)";
	$calcQry = mysql_query($calcSql) or die ("Error calcBagi :".mysql_error());
	$calcHsl = mysql_fetch_array($calcQry);
	$hasil = $calcHsl[0];
	return $hasil;
}
function calcKurangi($a, $b) {
	$calcSql = "SELECT FORMAT($a - $b, 6)";
	$calcQry = mysql_query($calcSql) or die ("Error calcKurangi :".mysql_error());
	$calcHsl = mysql_fetch_array($calcQry);
	$hasil = $calcHsl[0];
	return $hasil;
}
